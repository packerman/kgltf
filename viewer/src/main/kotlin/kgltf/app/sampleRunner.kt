...
typealias GltfTransformer = (Gltf) -> Gltf

private val sampleGltfTransformers: Map<KhronosSample, GltfTransformer> = mapOf(
...
private val variantSpecificTransformer: Map<KhronosSample, Map<Variant, GltfTransformer>> = mapOf(
        KhronosSample.Duck to mapOf(
                Variant.GltfTechniqueWebGL to { gltf ->
...
open class SampleApplicationRunner(config: Config, val sample: KhronosSample, val variant: Variant) : ApplicationRunner(config) {
    final override fun transformGltfModel(gltf: Gltf): Gltf {
        val transformer = variantSpecificTransformer[sample]?.get(variant) ?: sampleGltfTransformers[sample]
